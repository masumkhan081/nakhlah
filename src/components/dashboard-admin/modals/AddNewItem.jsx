...
const handleFormData = (title, values) => {
    ...
    if (title === 'purpose') {
        ...
        formData.append("data", `{"title":"${values.title}", "subtitle": "${values.subtitle}"}`)
    }
    return formData
}

// {title, addURL, addItemAPICall, errorMessageCall}
export default function AddNewItem({ title, isJourney }) {
    // console.log(title)

    const { toast } = useToast()
    const form = useForm({
        resolver: zodResolver(title === 'purpose' || title === 'level' ? formSchema : title === 'title' ? formSchema2 : formSchema3),
        defaultValues: title === 'purpose' || title === 'level' ? {
            title: '',
            picture: []
        } : title === 'title' ? {
            title: '',
    ...
    return (
        <>
            <DialogHeader>
                <DialogTitle className='textHeader textPrimaryColor'>GetStart Form</DialogTitle>
                <DialogDescription className='textNormal textSecondaryColor'>Input new {title} item</DialogDescription>
                        ...
                        {/* this form get start section  if you create new form for journey section create bool value is Journey or not  */}
                        {
                            isJourney ?
                                ...
                                <>
                                    ...
                                    <>
                                        {title === 'goal' && <InputField form={form} name={'time'} label={`Time`} placeholder={""} type={'number'} isAdmin={true} isItem={true} />}
                                    </>
                                    <>
                                        {
                                            title === 'title' &&
                                            <InputField form={form} name={'subtitle'} label={`Sub${title} type`} placeholder={""} type={'text'} isAdmin={true} isItem={true} />
                                        }
                                    </>
