...
const LeftNavbarItem = ({ item }) => {
    ...
    return (
        <div className=' textPrimaryColor pb-3'>
            <div onClick={() => handleClick(subLinks)} className={`pb-1 cursor-pointer hover:pl-2 transform duration-200 ease-in  ${subLinks != null && 'flex justify-between items-center'}`}>
                <div className='flex gap-3 items-center    text-[14px]'>
                    {icon}
                    {subLinks == null ? <Link href={link} className='pt-1 textNormal'>{title}</Link> : <div className='pt-1 textNormal cursor-pointer  '>
                        {title}
                    </div>
                    }
                </div>
                <span className={`${subLinks == null ? 'hidden' : 'block text-[14px] pt-1'}`}>{isClicked ? <BiChevronDown /> : <BiChevronRight />}</span>
            </div>
